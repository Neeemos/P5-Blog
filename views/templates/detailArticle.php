    ...
    <?php 
        if (empty($comments)) {
            ...
            foreach ($comments as $comment) {
                ...
                echo isset($_SESSION['user']) ? '<div class="admin"><a class="submit danger" href="index.php?action=deleteComment&article='.$comment->getIdArticle().'&id='.$comment->getId().'" onclick="return confirm(\'Êtes-vous sûr de vouloir supprimer ce commentaire ?\');">Supprimer</a></div>' : '';
                echo '  </div>';
                echo '</li>';
            }               
            echo '</ul>';
